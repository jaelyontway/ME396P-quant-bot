...
def load_margins_from_txt(filepath):
    """Load upper and lower margin values from output.txt"""
    upper_margin = None
    lower_margin = None
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line.startswith("UPPER_MARGIN:"):
                upper_margin = float(line.split(":")[1].strip())
            elif line.startswith("LOWER_MARGIN:"):
                lower_margin = float(line.split(":")[1].strip())
    
    if upper_margin is None or lower_margin is None:
        raise ValueError("Could not find UPPER_MARGIN or LOWER_MARGIN in output.txt")
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            try:
                ...
                price = float(row[3])  # UPDATED: price now in column D (index 3)
...
def plot_stock(stock: Stock, timestamps, interval_minutes=30):
    ...
    t = np.arange(len(prices)) * (interval_minutes / 60.0)
...
if __name__ == "__main__":
    ...
    # Load margins from output.txt
    upper_margin_value, lower_margin_value = load_margins_from_txt(MARGINS_FILE)
    
    # Set constant margin lines based on values from file
    s.set_upper_margin(upper_margin_value * np.ones_like(prices))
    s.set_lower_margin(lower_margin_value * np.ones_like(prices))

    with open(FILE_NAME, "r", encoding="utf-8") as f:
        reader = csv.reader(f)
        header = next(reader)
        all_rows = list(reader)
        start_date_str = all_rows[0][0].split()[0]
        start_date = datetime.strptime(start_date_str, "%Y-%m-%d")
        end_date_str = all_rows[-1][0].split()[0]
        end_date = datetime.strptime(end_date_str, "%Y-%m-%d")

    exit_index = plot_stock(s, timestamps, interval_minutes=60)

    plt.title(f"{s.get_name()} Price Simulation - {start_date.date()} → {end_date.date()} NYC Time")
    plt.show()
    ...
    print(f"Upper Margin: ${upper_margin_value:.2f}")
    print(f"Lower Margin: ${lower_margin_value:.2f}")
    if totalDiff >= 0:
